...
df_Sunset_Park <- df_NYC %>% filter(Zip%in% c(11220, 11232))
  df_Sunset_Park$Neighborhood <- "SUNSET PARK"
  
df_Bushwick_Williamsburg <- df_NYC %>% filter(Zip%in% c(11206, 11221, 12237))
...
df_Rockaways <- df_NYC %>% filter(Zip%in% c(11691, 11692, 11693, 11694, 11697))
  df_Rockaways$Neighborhood <- "ROCKAWAYS"  #11695 is a PO Box and is excluded
...
called_in <- read_csv("calledin.csv")

result <- merge(open_parking_violations, observed_violations, by = "Plate")

rounded_observed <- observed_violations

rounded_observed$latdd <- round(rounded_observed$latdd, digit = 4)
rounded_observed$longdd <- round(rounded_observed$longdd, digit = 4)

rounded_calledin <- called_in

rounded_calledin$latdd <- round(called_in$latdd, digit = 4)
rounded_calledin$longdd <- round(called_in$longdd, digit = 4)

rounded_overlap <- merge(rounded_observed, rounded_calledin, by = c("latdd", "longdd"))
...
register_google("AIzaSyBw3UEAd20Hs6sAbqqfkSsEFoJXVfPn04U") #Insert API Key here
...
Manhattan_calledin.map <- get_map(location= 'Manhattan, New York', 
                       maptype='roadmap', color='bw',source='google',zoom=12)
ggmap(Manhattan_calledin.map) + 
  geom_point(called_in, color='red', size=2, mapping = aes(x=longdd, y=latdd)) +
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  xlab('') + ylab('')

ggsave("calledin_manhattan.png", height = 1920, width = 1920, units = "px")

Manhattan_calledin_overlap.map <- get_map(location= 'Manhattan, New York', 
                       maptype='roadmap', color='bw',source='google',zoom=12)
ggmap(Manhattan_calledin_overlap.map) + 
  geom_point(called_in_overlap, color='red', size=2, mapping = aes(x=longdd, y=latdd)) +
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  xlab('') + ylab('')

ggsave("calledin_overlap_manhattan.png", height = 1920, width = 1920, units = "px")
