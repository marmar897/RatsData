...
rounded_observed$latdd <- round(rounded_observed$latdd, digit = 3)
rounded_observed$longdd <- round(rounded_observed$longdd, digit = 3)

rounded_calledin <- called_in

rounded_calledin$latdd <- round(called_in$latdd, digit = 3)
rounded_calledin$longdd <- round(called_in$longdd, digit = 3)
...
ggmap(Manhattan_calledin_overlap.map) + 
  geom_point(rounded_overlap, color='red', size=2, mapping = aes(x=longdd, y=latdd)) +
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  xlab('') + ylab('')
